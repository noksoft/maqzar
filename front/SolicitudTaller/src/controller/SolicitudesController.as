package controller
{
	...
	public class SolicitudesController extends AbstractController
	{
		...
		public function post_construct():void{
			...
			getFallasEquipo();
			//getProveedores();
			executeServiceCall(solicitudService.findProveedores(new ProveedorVO()), findProveedoresResult, findProveedoresFault);
		}
		
		private function findProveedoresResult (e:ResultEvent):void{
			Alert.show("ArrayCollection(e.result).length: " + ArrayCollection(e.result).length);
		}
		
		private function findProveedoresFault(e:FaultEvent):void{
		...
		private function getSolicitudes():void{
			executeServiceCall(solicitudService.findAllSolicitudes(new SolicitudVO()), findAllSolicitudesResult, findAllSolicitudesFault);
		}
		
		private function getProveedores():void{
			Alert.show("getProveedor");
		...
		private function getProveedoresResult(e:ResultEvent):void{
			try{
				Alert.show("getProveedorResult");
				if(e.result != null){
					solicitudModel.acProveedor = e.result as ArrayCollection;
					Alert.show("solicitudModel.acProveedor.length: " + solicitudModel.acProveedor.length);
				}else{
